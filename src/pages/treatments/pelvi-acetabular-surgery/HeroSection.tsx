export default function PelviAcetabularHero() {
  return (
    <section className="relative w-full min-h-[55vh] flex items-center justify-center overflow-hidden bg-secondary-600">
      <div className="absolute inset-0">
        <img
          src="https://readdy.ai/api/search-image?query=Orthopedic%20trauma%20surgeon%20reviewing%20pelvic%20X-ray%20scan%20in%20hospital%2C%20complex%20fracture%20analysis%2C%20advanced%20orthopedic%20trauma%20care%2C%20modern%20medical%20imaging%20review%2C%20realistic%20healthcare%20photography%2C%2016:9&width=1600&height=960&seq=phase6-pelvic-hero&orientation=landscape"
          alt="Pelvi-Acetabular Surgery - Dr. Sagar K V"
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto px-4 md:px-8 py-20 md:py-24 text-center">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/15 mb-6 reveal">
          <i className="ri-empathize-line text-rose-400 text-sm"></i>
          <span className="text-white/80 text-xs font-medium tracking-wide">Pelvi-Acetabular Surgery</span>
        </div>
        <h1 className="font-heading font-bold text-4xl sm:text-5xl md:text-6xl lg:text-7xl text-white leading-tight mb-4 reveal" style={{ transitionDelay: '100ms' }}>
          Pelvi-Acetabular{' '}
          <span className="text-rose-400">Surgery</span>
        </h1>
        <p className="text-white/55 text-base md:text-lg max-w-2xl mx-auto mb-6 reveal" style={{ transitionDelay: '200ms' }}>
          Complex pelvic and acetabular fracture reconstruction by a fellowship-trained specialist. One of the few surgeons in Bangalore with this advanced expertise.
        </p>
        <div className="flex flex-wrap items-center justify-center gap-6 md:gap-10 mb-8 reveal" style={{ transitionDelay: '300ms' }}>
          <div className="text-center"><span className="block font-heading font-bold text-3xl text-rose-300">15+</span><span className="text-white/45 text-xs">Years Experience</span></div>
          <div className="w-px h-10 bg-white/10" />
          <div className="text-center"><span className="block font-heading font-bold text-3xl text-rose-300">Fellowship</span><span className="text-white/45 text-xs">Max Hospital Mohali</span></div>
          <div className="w-px h-10 bg-white/10" />
          <div className="text-center"><span className="block font-heading font-bold text-3xl text-rose-300">90%+</span><span className="text-white/45 text-xs">Excellent Outcomes</span></div>
        </div>
